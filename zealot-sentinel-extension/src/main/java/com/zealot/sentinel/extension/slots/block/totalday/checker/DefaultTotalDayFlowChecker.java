...
public class DefaultTotalDayFlowChecker implements TotalDayFlowChecker {
	...
	public DefaultTotalDayFlowChecker(int totalDayCount)
	{
	...
	public boolean check(String resource, Context context, int count) {
		...
		AtomicInteger nowCount = map.get(key);
		
		if(null == nowCount)
		{
			synchronized (synConnFlag)
			{
		...
		int now = nowCount.incrementAndGet();
		return totalDayCount > now;
	}
